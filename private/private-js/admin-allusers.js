async function loadAllUsersAdmin() {
  ...
  document.querySelector(
    "#showing-panel"
  ).innerHTML += `<div class="col-md-12 text-center" id="title">USERS PANEL</div>`
  for (let user of usersDetail) {
    ...
    if (user.is_admin) {
      document.querySelector(
        "#showing-panel"
      ).innerHTML += `<div class="col-md-4 user-cards mb-2" userid="">
          <div class="card">
            <div class="card-body">
      ...
      document.querySelector(
        ...
        <div class="card">
          <div class="card-body">
            ...
            <a userid="${
              user.id
            }" class="btn btn-${buttonClass} ${statusButton}-btn">${statusButton}</a>
          </div>
        </div>
    </div>
    `
