...
account.get("/request/detail", async (req, res) => {
  ...
  const reqUser = await client.query(
    "select users_request.user_id, users_request.event_id, users.first_name, users.last_name, users.phone, events.name, users_request.id from users_request inner join users on users_request.user_id = users.id inner join events on users_request.event_id = events.id where users_request.organiser_id = $1 and processed = false order by users_request.id asc;",
    [userid]
  )
...
account.put("/profile", formidableMiddleware, async (req, res) => {
  ...
  if (image) {
    ...
    await client.query(
      "update users set first_name = $1, last_name = $2, phone = $3, birthday = $4, bio = $5 where id = $6 ",
      [firstName, lastName, phone, birthday, bio, userid]
    )
  } else {
    await client.query(
      "update users set first_name = $1, last_name = $2, phone = $3, birthday = $4, bio = $5 where id = $6 ",
